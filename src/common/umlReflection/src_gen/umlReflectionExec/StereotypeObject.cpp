...
{	
	m_StereotypeValue.reset();

	fUML::Semantics::StructuredClassifiers::ObjectImpl::destroy();
}

std::shared_ptr<uml::Element> StereotypeObject::getUmlValue() const
{
	return getStereotypeValue();
}

std::shared_ptr<uml::Stereotype> StereotypeObject::getStereotypeValue() const
{
	return m_StereotypeValue;
}

void StereotypeObject::setUmlValue(std::shared_ptr<uml::Element> _element)
{
	setStereotypeValue(std::dynamic_pointer_cast<uml::Stereotype>(_element));
}

void StereotypeObject::setStereotypeValue(std::shared_ptr<uml::Stereotype> _StereotypeElement)
{
	m_StereotypeValue = _StereotypeElement;
	//set super type values
	UML::ClassObject::setClassValue(_StereotypeElement);
}

void StereotypeObject::setThisStereotypeObjectPtr(std::weak_ptr<StereotypeObject> thisObjectPtr)
{
	setThisCS_ObjectPtr(thisObjectPtr);
}

bool StereotypeObject::equals(std::shared_ptr<fUML::Semantics::Values::Value> otherValue)
{
	bool isEqual = false;
	
	std::shared_ptr<StereotypeObject> otherStereotypeObject = std::dynamic_pointer_cast<StereotypeObject>(otherValue);

	if(otherStereotypeObject != nullptr)
	{
		if(this->getStereotypeValue() == otherStereotypeObject->getStereotypeValue())
		{
			isEqual = true;
		}
	}
...
std::string StereotypeObject::toString()
{
	std::string buffer = "Instance of 'StereotypeObject', " + std::to_string(getTypes()->size()) + " types (";
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Object>> StereotypeObject::getDirectContainers()
{   
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Link>> StereotypeObject::getLinks(std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_InteractionPoint> interactionPoint)
{
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> StereotypeObject::retrieveEndValueAsInteractionPoint(std::shared_ptr<fUML::Semantics::Values::Value> endValue, std::shared_ptr<uml::ConnectorEnd> end)
{
	/*Not done for metamodel object classes*/
...
void StereotypeObject::removeValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<fUML::Semantics::Values::Value> value)
{
	...
	if (m_StereotypeValue == nullptr)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": no value stored inside StereotypeObject (property: " << feature->getName() << ")" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_interfaceRealization())
	{
		...
		{
			m_StereotypeValue->getInterfaceRealization()->clear();
		}
		else
		{
			...
			{
				m_StereotypeValue->getInterfaceRealization()->erase(inputValue->getInterfaceRealizationValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_ownedBehavior())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedBehavior()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedBehavior()->erase(inputValue->getBehaviorValue());
			}
		}
	}
	...
	{
				m_StereotypeValue->setIsActive(false /*defined default value*/);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_nestedClassifier())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getNestedClassifier()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getNestedClassifier()->erase(inputValue->getClassifierValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedAttribute())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getClass_OwnedAttribute()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getClass_OwnedAttribute()->erase(inputValue->getPropertyValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedOperation())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedOperation()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedOperation()->erase(inputValue->getOperationValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedReception())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedReception()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedReception()->erase(inputValue->getReceptionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_superClass())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getSuperClass()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getSuperClass()->erase(inputValue->getClassValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_collaborationUse())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getCollaborationUse()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getCollaborationUse()->erase(inputValue->getCollaborationUseValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_generalization())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getGeneralization()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
	...
	{
				m_StereotypeValue->setIsFinalSpecialization(false /*defined default value*/);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedUseCase())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedUseCase()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedUseCase()->erase(inputValue->getUseCaseValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_powertypeExtent())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getPowertypeExtent()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getPowertypeExtent()->erase(inputValue->getGeneralizationSetValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_redefinedClassifier())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getRedefinedClassifier()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getRedefinedClassifier()->erase(inputValue->getClassifierValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_substitution())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getSubstitution()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getSubstitution()->erase(inputValue->getSubstitutionValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_useCase())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getUseCase()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getUseCase()->erase(inputValue->getUseCaseValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedComment()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedComment()->erase(inputValue->getCommentValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getElementImport()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getElementImport()->erase(inputValue->getElementImportValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedRule()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getOwnedRule()->erase(inputValue->getConstraintValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getPackageImport()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getPackageImport()->erase(inputValue->getPackageImportValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_PackageableElement_visibility())
	{
				m_StereotypeValue->setVisibility(uml::VisibilityKind::PUBLIC /*defined default value*/);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Stereotype_icon())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getIcon()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = std::dynamic_pointer_cast<PSCS::Semantics::StructuredClassifiers::CS_Reference>(value); */
			std::shared_ptr<fUML::Semantics::StructuredClassifiers::Reference> reference = std::dynamic_pointer_cast<fUML::Semantics::StructuredClassifiers::Reference>(value);
			std::shared_ptr<UML::ImageObject> inputValue = std::dynamic_pointer_cast<UML::ImageObject>(reference->getReferent());
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getIcon()->erase(inputValue->getImageValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_StructuredClassifier_ownedConnector())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getOwnedConnector()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_TemplateableElement_templateBinding())
	{
		if (value == nullptr) // clear mode
		{
			m_StereotypeValue->getTemplateBinding()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_StereotypeValue->getTemplateBinding()->erase(inputValue->getTemplateBindingValue());
			}
		}
	}
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> StereotypeObject::getValues(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> featureValues)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_classifierBehavior())
	{
		...
		value->setBehaviorValue(m_StereotypeValue->getClassifierBehavior());
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_interfaceRealization())
	{
		std::shared_ptr<Subset<uml::InterfaceRealization, uml::Element>> interfaceRealizationList = m_StereotypeValue->getInterfaceRealization();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_ownedBehavior())
	{
		std::shared_ptr<SubsetUnion<uml::Behavior, uml::NamedElement>> ownedBehaviorList = m_StereotypeValue->getOwnedBehavior();
	...
	{
		std::shared_ptr<Bag<uml::Extension>> extensionList = m_StereotypeValue->getExtension();
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_StereotypeValue->getIsActive());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_nestedClassifier())
	{
		std::shared_ptr<Subset<uml::Classifier, uml::NamedElement>> nestedClassifierList = m_StereotypeValue->getNestedClassifier();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedAttribute())
	{
		std::shared_ptr<Subset<uml::Property, uml::ConnectableElement, uml::NamedElement, uml::Property>> ownedAttributeList = m_StereotypeValue->getClass_OwnedAttribute();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedOperation())
	{
		std::shared_ptr<Subset<uml::Operation, uml::Feature, uml::NamedElement>> ownedOperationList = m_StereotypeValue->getOwnedOperation();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedReception())
	{
		std::shared_ptr<Subset<uml::Reception, uml::Feature, uml::NamedElement>> ownedReceptionList = m_StereotypeValue->getOwnedReception();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_superClass())
	{
		std::shared_ptr<Bag<uml::Class>> superClassList = m_StereotypeValue->getSuperClass();
	...
	{
		std::shared_ptr<SubsetUnion<uml::Property, uml::Feature>> attributeList = m_StereotypeValue->getAttribute();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_collaborationUse())
	{
		std::shared_ptr<SubsetUnion<uml::CollaborationUse, uml::Element>> collaborationUseList = m_StereotypeValue->getCollaborationUse();
	...
	{
		std::shared_ptr<SubsetUnion<uml::Feature, uml::NamedElement>> featureList = m_StereotypeValue->getFeature();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_generalization())
	{
		std::shared_ptr<Subset<uml::Generalization, uml::Element>> generalizationList = m_StereotypeValue->getGeneralization();
	...
	{
		std::shared_ptr<Subset<uml::NamedElement, uml::NamedElement>> inheritedMemberList = m_StereotypeValue->getInheritedMember();
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_StereotypeValue->getIsFinalSpecialization());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedTemplateSignature())
	{
		...
		value->setLocus(this->getLocus());
		value->setRedefinableTemplateSignatureValue(std::dynamic_pointer_cast<uml::RedefinableTemplateSignature>(m_StereotypeValue->getOwnedTemplateSignature()));
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedUseCase())
	{
		std::shared_ptr<Subset<uml::UseCase, uml::NamedElement>> ownedUseCaseList = m_StereotypeValue->getOwnedUseCase();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_powertypeExtent())
	{
		std::shared_ptr<Bag<uml::GeneralizationSet>> powertypeExtentList = m_StereotypeValue->getPowertypeExtent();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_redefinedClassifier())
	{
		std::shared_ptr<SubsetUnion<uml::Classifier, uml::RedefinableElement>> redefinedClassifierList = m_StereotypeValue->getRedefinedClassifier();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_representation())
	{
		...
		value->setThisCollaborationUseObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setCollaborationUseValue(m_StereotypeValue->getRepresentation());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_substitution())
	{
		std::shared_ptr<Subset<uml::Substitution, uml::Element>> substitutionList = m_StereotypeValue->getSubstitution();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_templateParameter())
	{
		...
		value->setLocus(this->getLocus());
		value->setClassifierTemplateParameterValue(std::dynamic_pointer_cast<uml::ClassifierTemplateParameter>(m_StereotypeValue->getTemplateParameter()));
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_useCase())
	{
		std::shared_ptr<Bag<uml::UseCase>> useCaseList = m_StereotypeValue->getUseCase();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		std::shared_ptr<Subset<uml::Comment, uml::Element>> ownedCommentList = m_StereotypeValue->getOwnedComment();
	...
	{
		std::shared_ptr<Union<uml::Element>> ownedElementList = m_StereotypeValue->getOwnedElement();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_owner())
	{
		...
		value->setThisElementObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setElementValue(m_StereotypeValue->getOwner().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Subset<uml::Port, uml::Property /*Subset does not reference a union*/>> ownedPortList = m_StereotypeValue->getOwnedPort();
	...
	{
		std::shared_ptr<Bag<uml::Dependency>> clientDependencyList = m_StereotypeValue->getClientDependency();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		value->setValue(m_StereotypeValue->getName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		value->setLocus(this->getLocus());
		value->setStringExpressionValue(m_StereotypeValue->getNameExpression());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace())
	{
		...
		value->setLocus(this->getLocus());
		value->setNamespaceValue(m_StereotypeValue->getNamespace().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::StringValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createStringValue();
		value->setValue(m_StereotypeValue->getQualifiedName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		uml::VisibilityKind visibility = m_StereotypeValue->getVisibility();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport())
	{
		std::shared_ptr<SubsetUnion<uml::ElementImport, uml::Element>> elementImportList = m_StereotypeValue->getElementImport();
	...
	{
		std::shared_ptr<Subset<uml::PackageableElement, uml::NamedElement>> importedMemberList = m_StereotypeValue->getImportedMember();
	...
	{
		std::shared_ptr<Union<uml::NamedElement>> memberList = m_StereotypeValue->getMember();
	...
	{
		std::shared_ptr<SubsetUnion<uml::NamedElement, uml::Element, uml::NamedElement>> ownedMemberList = m_StereotypeValue->getOwnedMember();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule())
	{
		std::shared_ptr<SubsetUnion<uml::Constraint, uml::NamedElement>> ownedRuleList = m_StereotypeValue->getOwnedRule();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport())
	{
		std::shared_ptr<SubsetUnion<uml::PackageImport, uml::Element>> packageImportList = m_StereotypeValue->getPackageImport();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ParameterableElement_owningTemplateParameter())
	{
		...
		value->setLocus(this->getLocus());
		value->setTemplateParameterValue(m_StereotypeValue->getOwningTemplateParameter().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_StereotypeValue->getIsLeaf());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement())
	{
		std::shared_ptr<Union<uml::RedefinableElement>> redefinedElementList = m_StereotypeValue->getRedefinedElement();
	...
	{
		std::shared_ptr<Union<uml::Classifier>> redefinitionContextList = m_StereotypeValue->getRedefinitionContext();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Stereotype_icon())
	{
		std::shared_ptr<Subset<uml::Image, uml::Element>> iconList = m_StereotypeValue->getIcon();
		Bag<uml::Image>::iterator iter = iconList->begin();
		Bag<uml::Image>::iterator end = iconList->end();
		while (iter != end)
		{
			std::shared_ptr<UML::ImageObject> value(new UML::ImageObject());
			value->setThisImageObjectPtr(value);
			value->setLocus(this->getLocus());
			value->setImageValue(*iter);
			std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
			reference->setReferent(value);
			reference->setCompositeReferent(value);
			values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Stereotype_profile())
	{
		std::shared_ptr<UML::ProfileObject> value(new UML::ProfileObject());
		value->setThisProfileObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setProfileValue(m_StereotypeValue->getProfile());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_StructuredClassifier_ownedAttribute())
	{
		std::shared_ptr<SubsetUnion<uml::Property, uml::ConnectableElement, uml::NamedElement, uml::Property>> ownedAttributeList = m_StereotypeValue->getOwnedAttribute();
		Bag<uml::Property>::iterator iter = ownedAttributeList->begin();
		Bag<uml::Property>::iterator end = ownedAttributeList->end();
		while (iter != end)
		{
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_StructuredClassifier_ownedConnector())
	{
		std::shared_ptr<Subset<uml::Connector, uml::Feature, uml::NamedElement>> ownedConnectorList = m_StereotypeValue->getOwnedConnector();
	...
	{
		std::shared_ptr<Bag<uml::Property>> partList = m_StereotypeValue->getPart();
	...
	{
		std::shared_ptr<SubsetUnion<uml::ConnectableElement, uml::NamedElement>> roleList = m_StereotypeValue->getRole();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_TemplateableElement_templateBinding())
	{
		std::shared_ptr<Subset<uml::TemplateBinding, uml::Element>> templateBindingList = m_StereotypeValue->getTemplateBinding();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Type_package())
	{
		...
		value->setLocus(this->getLocus());
		value->setPackageValue(m_StereotypeValue->getPackage().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
...
std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue> StereotypeObject::retrieveFeatureValue(std::shared_ptr<uml::StructuralFeature> feature)
{
...
void StereotypeObject::setFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_classifierBehavior())
	{
		...
		if (value != nullptr)
		{
			m_StereotypeValue->setClassifierBehavior(value->getBehaviorValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_interfaceRealization())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getInterfaceRealization()->push_back(value->getInterfaceRealizationValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_ownedBehavior())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedBehavior()->push_back(value->getBehaviorValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_isActive())
	{
		...
		if (valueObject != nullptr)
		{
			m_StereotypeValue->setIsActive(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_nestedClassifier())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getNestedClassifier()->push_back(value->getClassifierValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedAttribute())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getClass_OwnedAttribute()->push_back(value->getPropertyValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedOperation())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedOperation()->push_back(value->getOperationValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_ownedReception())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedReception()->push_back(value->getReceptionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Class_superClass())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getSuperClass()->push_back(value->getClassValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_collaborationUse())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getCollaborationUse()->push_back(value->getCollaborationUseValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_generalization())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getGeneralization()->push_back(value->getGeneralizationValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_isFinalSpecialization())
	{
		...
		if (valueObject != nullptr)
		{
			m_StereotypeValue->setIsFinalSpecialization(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedTemplateSignature())
	{
		...
		if (value != nullptr)
		{
			m_StereotypeValue->setOwnedTemplateSignature(value->getRedefinableTemplateSignatureValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedUseCase())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedUseCase()->push_back(value->getUseCaseValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_powertypeExtent())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getPowertypeExtent()->push_back(value->getGeneralizationSetValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_redefinedClassifier())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getRedefinedClassifier()->push_back(value->getClassifierValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_representation())
	{
		...
		if (value != nullptr)
		{
			m_StereotypeValue->setRepresentation(value->getCollaborationUseValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_substitution())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getSubstitution()->push_back(value->getSubstitutionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_templateParameter())
	{
		...
		if (value != nullptr)
		{
			m_StereotypeValue->setTemplateParameter(value->getClassifierTemplateParameterValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Classifier_useCase())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getUseCase()->push_back(value->getUseCaseValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedComment()->push_back(value->getCommentValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		if (valueObject != nullptr)
		{
			m_StereotypeValue->setName(valueObject->getValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		if (value != nullptr)
		{
			m_StereotypeValue->setNameExpression(value->getStringExpressionValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		...
		{
			m_StereotypeValue->setVisibility(uml::VisibilityKind::PUBLIC);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_private())
		{
			m_StereotypeValue->setVisibility(uml::VisibilityKind::PRIVATE);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_protected())
		{
			m_StereotypeValue->setVisibility(uml::VisibilityKind::PROTECTED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_package())
		{
			m_StereotypeValue->setVisibility(uml::VisibilityKind::PACKAGE);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getElementImport()->push_back(value->getElementImportValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedRule()->push_back(value->getConstraintValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getPackageImport()->push_back(value->getPackageImportValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		if (valueObject != nullptr)
		{
			m_StereotypeValue->setIsLeaf(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Stereotype_icon())
	{
		...
		while (iter != end)
		{
			std::shared_ptr<fUML::Semantics::Values::Value> inputValue = *iter;
			std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = std::dynamic_pointer_cast<PSCS::Semantics::StructuredClassifiers::CS_Reference>(inputValue);
			std::shared_ptr<UML::ImageObject> value = std::dynamic_pointer_cast<UML::ImageObject>(reference->getReferent());
			if (value != nullptr)
			{
				m_StereotypeValue->getIcon()->push_back(value->getImageValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_StructuredClassifier_ownedConnector())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getOwnedConnector()->push_back(value->getConnectorValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_TemplateableElement_templateBinding())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_StereotypeValue->getTemplateBinding()->push_back(value->getTemplateBindingValue());
			}
...
void StereotypeObject::assignFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	this->setFeatureValue(feature, values, position);
}

std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> StereotypeObject::retrieveFeatureValues()
{
	...
	for(unsigned int i = 0; i < allAttributesSize; i++)
	{
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_classifierBehavior() && m_StereotypeValue->getClassifierBehavior() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_interfaceRealization() && m_StereotypeValue->getInterfaceRealization() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_BehavioredClassifier_ownedBehavior() && m_StereotypeValue->getOwnedBehavior() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_extension() && m_StereotypeValue->getExtension() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_nestedClassifier() && m_StereotypeValue->getNestedClassifier() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_ownedAttribute() && m_StereotypeValue->getClass_OwnedAttribute() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_ownedOperation() && m_StereotypeValue->getOwnedOperation() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_ownedReception() && m_StereotypeValue->getOwnedReception() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Class_superClass() && m_StereotypeValue->getSuperClass() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_attribute() && m_StereotypeValue->getAttribute() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_collaborationUse() && m_StereotypeValue->getCollaborationUse() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_generalization() && m_StereotypeValue->getGeneralization() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_inheritedMember() && m_StereotypeValue->getInheritedMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedTemplateSignature() && m_StereotypeValue->getOwnedTemplateSignature() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_ownedUseCase() && m_StereotypeValue->getOwnedUseCase() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_powertypeExtent() && m_StereotypeValue->getPowertypeExtent() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_redefinedClassifier() && m_StereotypeValue->getRedefinedClassifier() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_representation() && m_StereotypeValue->getRepresentation() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_substitution() && m_StereotypeValue->getSubstitution() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_templateParameter() && m_StereotypeValue->getTemplateParameter() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Classifier_useCase() && m_StereotypeValue->getUseCase() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment() && m_StereotypeValue->getOwnedComment() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedElement() && m_StereotypeValue->getOwnedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_owner() && m_StereotypeValue->getOwner().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_EncapsulatedClassifier_ownedPort() && m_StereotypeValue->getOwnedPort() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_clientDependency() && m_StereotypeValue->getClientDependency() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression() && m_StereotypeValue->getNameExpression() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace() && m_StereotypeValue->getNamespace().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport() && m_StereotypeValue->getElementImport() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_importedMember() && m_StereotypeValue->getImportedMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_member() && m_StereotypeValue->getMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedMember() && m_StereotypeValue->getOwnedMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule() && m_StereotypeValue->getOwnedRule() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport() && m_StereotypeValue->getPackageImport() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ParameterableElement_owningTemplateParameter() && m_StereotypeValue->getOwningTemplateParameter().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinitionContext() && m_StereotypeValue->getRedefinitionContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Stereotype_icon() && m_StereotypeValue->getIcon() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Stereotype_profile() && m_StereotypeValue->getProfile() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_StructuredClassifier_part() && m_StereotypeValue->getPart() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_TemplateableElement_templateBinding() && m_StereotypeValue->getTemplateBinding() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Type_package() && m_StereotypeValue->getPackage().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
	}
