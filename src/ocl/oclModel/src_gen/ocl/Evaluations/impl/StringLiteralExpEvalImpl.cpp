...
StringLiteralExpEvalImpl& StringLiteralExpEvalImpl::operator=(const StringLiteralExpEvalImpl & obj)
{
	 ...
	 * Currently, this causes an error because it calls an implicit assignment operator of StringLiteralExpEval 
	 * which is generated by the compiler (as StringLiteralExpEval is an abstract class and does not have a user-defined assignment operator).
...
bool StringLiteralExpEvalImpl::eSet(int featureID, const Any& newValue)
{
	switch(featureID)
	{
	}
...
// EOperation Invoke
//*********************************
Any StringLiteralExpEvalImpl::eInvoke(int operationID, const std::shared_ptr<std::list<Any>>& arguments)
{
	Any result;
