...
if [[ -x "${ECLIPSE_BIN}" ]]; then
  ...
  if [[ "${acceleo_version}" == "${MDE4CPP_ECLIPSE_ACCELEO_VERSION//[[:space:]]/}"* && "${sirius_version}" == "${MDE4CPP_ECLIPSE_SIRIUS_VERSION//[[:space:]]/}"* && -n "${cdt_version}" ]]; then
...
"${ECLIPSE_BIN}" \
  ...
  -profileProperties org.eclipse.update.install.features=true 2> "${TMP_DIR}/p2_stderr.log"
