...
echo %C_PURPLE%[install_cmake]%C_ORANGE% MDE4CPP_CMAKE_VERSION=!MDE4CPP_CMAKE_VERSION!%C_RESET%
echo %C_PURPLE%[install_cmake]%C_ORANGE% MDE4CPP_CMAKE_BUILD_VERSION=!MDE4CPP_CMAKE_BUILD_VERSION!%C_RESET%
...
if not "%INSTALLED_VERSION%"=="" (
    if "%INSTALLED_VERSION%"=="%CMAKE_FULL_VERSION%" (
        ...
        echo %C_PURPLE%[install_cmake]%C_SUCCESS% Found installed CMake %INSTALLED_VERSION%. Installing %CMAKE_FULL_VERSION%.%C_RESET%
    )
) else (
    echo %C_PURPLE%[install_cmake]%C_INFO% CMake is not installed. Installing %CMAKE_FULL_VERSION%.%C_RESET%
)
...
net session >nul 2>&1
if errorlevel 1 (
    echo %C_PURPLE%[install_cmake]%C_INFO% Administrator rights are required. Requesting elevation...%C_RESET%
    ...
    if not "!ELEV_ERR!"=="0" (
        echo %C_PURPLE%[install_cmake]%C_ERROR% ERROR: Elevation was cancelled or failed.%C_RESET%
        exit /b 1
    )
    ...
    if defined VERIFY_CMAKE (
        echo %C_PURPLE%[install_cmake]%C_SUCCESS% Verified: CMake !VERIFY_CMAKE! installed.%C_RESET%
        exit /b 0
    ) else (
        echo %C_PURPLE%[install_cmake]%C_INFO% WARNING: Could not verify CMake after elevation. It may require a new shell.%C_RESET%
        exit /b 0
    )
)
...
echo %C_PURPLE%[install_cmake]%C_INFO% Install mode=system application%C_RESET%
echo %C_PURPLE%[install_cmake]%C_INFO% Using CMake version=%CMAKE_FULL_VERSION%%C_RESET%
echo %C_PURPLE%[install_cmake]%C_INFO% Downloading %DOWNLOAD_URL%%C_RESET%

if exist "%TMP_DIR%" rmdir /s /q "%TMP_DIR%"
mkdir "%TMP_DIR%" >nul 2>&1
if errorlevel 1 (
    echo %C_PURPLE%[install_cmake]%C_ERROR% ERROR: Failed to create temp dir %TMP_DIR%.%C_RESET%
    exit /b 1
)

call "%~dp0common.bat" download_file "%DOWNLOAD_URL%" "%INSTALLER_PATH%"
if errorlevel 1 (
    rmdir /s /q "%TMP_DIR%"
    exit /b 1
)
...
if errorlevel 1 (
    echo %C_PURPLE%[install_cmake]%C_ERROR% ERROR: MSI installation failed. Try running as Administrator.%C_RESET%
    rmdir /s /q "%TMP_DIR%"
    exit /b 1
)
...
rmdir /s /q "%TMP_DIR%"
echo %C_PURPLE%[install_cmake]%C_INFO% Installed system CMake %CMAKE_FULL_VERSION%%C_RESET%
where cmake >nul 2>&1
if errorlevel 1 (
    echo %C_PURPLE%[install_cmake]%C_INFO% WARNING: cmake is not on PATH yet. Open a new shell.%C_RESET%
) else (
    for /f "delims=" %%P in ('where cmake') do (
        echo %C_PURPLE%[install_cmake]%C_INFO% Binary: %%P%C_RESET%
        goto :done
    )
)
:done
