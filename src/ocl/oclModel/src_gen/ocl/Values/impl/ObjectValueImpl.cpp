...
ObjectValueImpl& ObjectValueImpl::operator=(const ObjectValueImpl & obj)
{
	 ...
	 * Currently, this causes an error because it calls an implicit assignment operator of ObjectValue 
	 * which is generated by the compiler (as ObjectValue is an abstract class and does not have a user-defined assignment operator).
