...
{
	std::shared_ptr<ecore::EObject> element(new DestroyLinkActionImpl(*this));
	return element;
}
...
void DestroyLinkActionImpl::eSet(int featureID, boost::any newValue)
{
	switch(featureID)
	{
		case UmlPackage::ACTIVITYNODE_EREFERENCE_ACTIVITY:
		...
		case UmlPackage::ACTIVITYNODE_EREFERENCE_INSTRUCTUREDNODE:
		{
			...
			setInStructuredNode(_inStructuredNode); //15116
			break;
		}
		case UmlPackage::REDEFINABLEELEMENT_EATTRIBUTE_ISLEAF:
		{
		...
		case UmlPackage::ACTION_EATTRIBUTE_ISLOCALLYREENTRANT:
		{
			...
			setIsLocallyReentrant(_isLocallyReentrant); //15124
			break;
		}
		case UmlPackage::NAMEDELEMENT_EATTRIBUTE_NAME:
		{
		...
		case UmlPackage::NAMEDELEMENT_EREFERENCE_NAMEEXPRESSION:
		{
			...
			setNameExpression(_nameExpression); //1516
			break;
		}
		case UmlPackage::NAMEDELEMENT_EATTRIBUTE_VISIBILITY:
		{
			...
			setVisibility(_visibility); //1519
			break;
		}
	}
}
