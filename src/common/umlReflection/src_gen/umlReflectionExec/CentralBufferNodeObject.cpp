...
{	
	m_CentralBufferNodeValue.reset();

	fUML::Semantics::StructuredClassifiers::ObjectImpl::destroy();
}

std::shared_ptr<uml::Element> CentralBufferNodeObject::getUmlValue() const
{
	return getCentralBufferNodeValue();
}

std::shared_ptr<uml::CentralBufferNode> CentralBufferNodeObject::getCentralBufferNodeValue() const
{
	return m_CentralBufferNodeValue;
}

void CentralBufferNodeObject::setUmlValue(std::shared_ptr<uml::Element> _element)
{
	setCentralBufferNodeValue(std::dynamic_pointer_cast<uml::CentralBufferNode>(_element));
}

void CentralBufferNodeObject::setCentralBufferNodeValue(std::shared_ptr<uml::CentralBufferNode> _CentralBufferNodeElement)
{
	m_CentralBufferNodeValue = _CentralBufferNodeElement;
	//set super type values
	UML::ObjectNodeObject::setObjectNodeValue(_CentralBufferNodeElement);
}

void CentralBufferNodeObject::setThisCentralBufferNodeObjectPtr(std::weak_ptr<CentralBufferNodeObject> thisObjectPtr)
{
	setThisCS_ObjectPtr(thisObjectPtr);
}

bool CentralBufferNodeObject::equals(std::shared_ptr<fUML::Semantics::Values::Value> otherValue)
{
	bool isEqual = false;
	
	std::shared_ptr<CentralBufferNodeObject> otherCentralBufferNodeObject = std::dynamic_pointer_cast<CentralBufferNodeObject>(otherValue);

	if(otherCentralBufferNodeObject != nullptr)
	{
		if(this->getCentralBufferNodeValue() == otherCentralBufferNodeObject->getCentralBufferNodeValue())
		{
			isEqual = true;
		}
	}
...
std::string CentralBufferNodeObject::toString()
{
	std::string buffer = "Instance of 'CentralBufferNodeObject', " + std::to_string(getTypes()->size()) + " types (";
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Object>> CentralBufferNodeObject::getDirectContainers()
{   
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Link>> CentralBufferNodeObject::getLinks(std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_InteractionPoint> interactionPoint)
{
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> CentralBufferNodeObject::retrieveEndValueAsInteractionPoint(std::shared_ptr<fUML::Semantics::Values::Value> endValue, std::shared_ptr<uml::ConnectorEnd> end)
{
	/*Not done for metamodel object classes*/
...
void CentralBufferNodeObject::removeValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<fUML::Semantics::Values::Value> value)
{
	...
	if (m_CentralBufferNodeValue == nullptr)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": no value stored inside CentralBufferNodeObject (property: " << feature->getName() << ")" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		...
		{
			m_CentralBufferNodeValue->getInInterruptibleRegion()->clear();
		}
		else
		{
			...
			{
				m_CentralBufferNodeValue->getInInterruptibleRegion()->erase(inputValue->getInterruptibleActivityRegionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getInPartition()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getInPartition()->erase(inputValue->getActivityPartitionValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getIncoming()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getIncoming()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getOutgoing()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getOutgoing()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getRedefinedNode()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getRedefinedNode()->erase(inputValue->getActivityNodeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getOwnedComment()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getOwnedComment()->erase(inputValue->getCommentValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_inState())
	{
		if (value == nullptr) // clear mode
		{
			m_CentralBufferNodeValue->getInState()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_CentralBufferNodeValue->getInState()->erase(inputValue->getStateValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_isControlType())
	{
				m_CentralBufferNodeValue->setIsControlType(false /*defined default value*/);

	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_ordering())
	{
				m_CentralBufferNodeValue->setOrdering(uml::ObjectNodeOrderingKind::FIFO /*defined default value*/);
	...
	{
				m_CentralBufferNodeValue->setIsLeaf(false /*defined default value*/);
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> CentralBufferNodeObject::getValues(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> featureValues)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity())
	{
		...
		value->setActivityValue(m_CentralBufferNodeValue->getActivity().lock());
	...
	{
		std::shared_ptr<Union<uml::ActivityGroup>> inGroupList = m_CentralBufferNodeValue->getInGroup();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		std::shared_ptr<Subset<uml::InterruptibleActivityRegion, uml::ActivityGroup>> inInterruptibleRegionList = m_CentralBufferNodeValue->getInInterruptibleRegion();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		std::shared_ptr<Subset<uml::ActivityPartition, uml::ActivityGroup>> inPartitionList = m_CentralBufferNodeValue->getInPartition();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode())
	{
		...
		value->setLocus(this->getLocus());
		value->setStructuredActivityNodeValue(m_CentralBufferNodeValue->getInStructuredNode().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> incomingList = m_CentralBufferNodeValue->getIncoming();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> outgoingList = m_CentralBufferNodeValue->getOutgoing();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		std::shared_ptr<Subset<uml::ActivityNode, uml::RedefinableElement>> redefinedNodeList = m_CentralBufferNodeValue->getRedefinedNode();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		std::shared_ptr<Subset<uml::Comment, uml::Element>> ownedCommentList = m_CentralBufferNodeValue->getOwnedComment();
	...
	{
		std::shared_ptr<Union<uml::Element>> ownedElementList = m_CentralBufferNodeValue->getOwnedElement();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_owner())
	{
		...
		value->setThisElementObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setElementValue(m_CentralBufferNodeValue->getOwner().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Bag<uml::Dependency>> clientDependencyList = m_CentralBufferNodeValue->getClientDependency();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		value->setValue(m_CentralBufferNodeValue->getName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		value->setLocus(this->getLocus());
		value->setStringExpressionValue(m_CentralBufferNodeValue->getNameExpression());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace())
	{
		...
		value->setLocus(this->getLocus());
		value->setNamespaceValue(m_CentralBufferNodeValue->getNamespace().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::StringValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createStringValue();
		value->setValue(m_CentralBufferNodeValue->getQualifiedName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		uml::VisibilityKind visibility = m_CentralBufferNodeValue->getVisibility();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_inState())
	{
		std::shared_ptr<Bag<uml::State>> inStateList = m_CentralBufferNodeValue->getInState();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_isControlType())
	{
		...
		value->setValue(m_CentralBufferNodeValue->getIsControlType());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_ordering())
	{
		uml::ObjectNodeOrderingKind ordering = m_CentralBufferNodeValue->getOrdering();
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::EnumerationValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createEnumerationValue();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_selection())
	{
		...
		value->setLocus(this->getLocus());
		value->setBehaviorValue(m_CentralBufferNodeValue->getSelection());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_upperBound())
	{
		...
		value->setLocus(this->getLocus());
		value->setValueSpecificationValue(m_CentralBufferNodeValue->getUpperBound());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_CentralBufferNodeValue->getIsLeaf());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement())
	{
		std::shared_ptr<Union<uml::RedefinableElement>> redefinedElementList = m_CentralBufferNodeValue->getRedefinedElement();
	...
	{
		std::shared_ptr<Union<uml::Classifier>> redefinitionContextList = m_CentralBufferNodeValue->getRedefinitionContext();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_TypedElement_type())
	{
		...
		value->setLocus(this->getLocus());
		value->setTypeValue(m_CentralBufferNodeValue->getType());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
...
std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue> CentralBufferNodeObject::retrieveFeatureValue(std::shared_ptr<uml::StructuralFeature> feature)
{
...
void CentralBufferNodeObject::setFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getInInterruptibleRegion()->push_back(value->getInterruptibleActivityRegionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getInPartition()->push_back(value->getActivityPartitionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getIncoming()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getOutgoing()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getRedefinedNode()->push_back(value->getActivityNodeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getOwnedComment()->push_back(value->getCommentValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		{
			m_CentralBufferNodeValue->setName(valueObject->getValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		if (value != nullptr)
		{
			m_CentralBufferNodeValue->setNameExpression(value->getStringExpressionValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		...
		{
			m_CentralBufferNodeValue->setVisibility(uml::VisibilityKind::PUBLIC);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_private())
		{
			m_CentralBufferNodeValue->setVisibility(uml::VisibilityKind::PRIVATE);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_protected())
		{
			m_CentralBufferNodeValue->setVisibility(uml::VisibilityKind::PROTECTED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_package())
		{
			m_CentralBufferNodeValue->setVisibility(uml::VisibilityKind::PACKAGE);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_inState())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_CentralBufferNodeValue->getInState()->push_back(value->getStateValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_isControlType())
	{
		...
		if (valueObject != nullptr)
		{
			m_CentralBufferNodeValue->setIsControlType(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_ordering())
	{
		...
		{
			m_CentralBufferNodeValue->setOrdering(uml::ObjectNodeOrderingKind::UNORDERED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_ObjectNodeOrderingKind_ordered())
		{
			m_CentralBufferNodeValue->setOrdering(uml::ObjectNodeOrderingKind::ORDERED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_ObjectNodeOrderingKind_LIFO())
		{
			m_CentralBufferNodeValue->setOrdering(uml::ObjectNodeOrderingKind::LIFO);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_ObjectNodeOrderingKind_FIFO())
		{
			m_CentralBufferNodeValue->setOrdering(uml::ObjectNodeOrderingKind::FIFO);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_selection())
	{
		...
		if (value != nullptr)
		{
			m_CentralBufferNodeValue->setSelection(value->getBehaviorValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ObjectNode_upperBound())
	{
		...
		if (value != nullptr)
		{
			m_CentralBufferNodeValue->setUpperBound(value->getValueSpecificationValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		if (valueObject != nullptr)
		{
			m_CentralBufferNodeValue->setIsLeaf(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_TypedElement_type())
	{
		...
		if (value != nullptr)
		{
			m_CentralBufferNodeValue->setType(value->getTypeValue());
		}
		
	}
}

void CentralBufferNodeObject::assignFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	this->setFeatureValue(feature, values, position);
}

std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> CentralBufferNodeObject::retrieveFeatureValues()
{
	...
	for(unsigned int i = 0; i < allAttributesSize; i++)
	{
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity() && m_CentralBufferNodeValue->getActivity().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inGroup() && m_CentralBufferNodeValue->getInGroup() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion() && m_CentralBufferNodeValue->getInInterruptibleRegion() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition() && m_CentralBufferNodeValue->getInPartition() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode() && m_CentralBufferNodeValue->getInStructuredNode().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming() && m_CentralBufferNodeValue->getIncoming() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing() && m_CentralBufferNodeValue->getOutgoing() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode() && m_CentralBufferNodeValue->getRedefinedNode() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment() && m_CentralBufferNodeValue->getOwnedComment() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedElement() && m_CentralBufferNodeValue->getOwnedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_owner() && m_CentralBufferNodeValue->getOwner().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_clientDependency() && m_CentralBufferNodeValue->getClientDependency() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression() && m_CentralBufferNodeValue->getNameExpression() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace() && m_CentralBufferNodeValue->getNamespace().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ObjectNode_inState() && m_CentralBufferNodeValue->getInState() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ObjectNode_selection() && m_CentralBufferNodeValue->getSelection() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ObjectNode_upperBound() && m_CentralBufferNodeValue->getUpperBound() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement() && m_CentralBufferNodeValue->getRedefinedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinitionContext() && m_CentralBufferNodeValue->getRedefinitionContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_TypedElement_type() && m_CentralBufferNodeValue->getType() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
	}
