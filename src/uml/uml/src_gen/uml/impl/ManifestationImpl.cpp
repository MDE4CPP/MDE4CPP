	...
	switch(featureID)
	{
		case uml::umlPackage::MANIFESTATION_ATTRIBUTE_UTILIZEDELEMENT:
		{
			...
			if(ecoreAny)
			{
				...
				{
					DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid type stored in 'ecore::ecoreAny' for feature 'utilizedElement'. Failed to set feature!"<< std::endl;)
					return false;
				}
			}
			else
			{
				DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid instance of 'ecore::ecoreAny' for feature 'utilizedElement'. Failed to set feature!"<< std::endl;)
				return false;
			}
		return true;
		}
