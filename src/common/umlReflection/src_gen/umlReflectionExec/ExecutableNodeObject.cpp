...
{	
	if(m_ExecutableNodeValue)
	{

	}

	m_ExecutableNodeValue.reset();

	fUML::Semantics::StructuredClassifiers::ObjectImpl::destroy();
}

std::shared_ptr<uml::Element> ExecutableNodeObject::getUmlValue() const
{
	return getExecutableNodeValue();
}

std::shared_ptr<uml::ExecutableNode> ExecutableNodeObject::getExecutableNodeValue() const
{
	return m_ExecutableNodeValue;
}

void ExecutableNodeObject::setUmlValue(std::shared_ptr<uml::Element> _element)
{
	setExecutableNodeValue(std::dynamic_pointer_cast<uml::ExecutableNode>(_element));
}

void ExecutableNodeObject::setExecutableNodeValue(std::shared_ptr<uml::ExecutableNode> _ExecutableNodeElement)
{
	m_ExecutableNodeValue = _ExecutableNodeElement;
	//set super type values
	UML::ActivityNodeObject::setActivityNodeValue(_ExecutableNodeElement);
}

void ExecutableNodeObject::setThisExecutableNodeObjectPtr(std::weak_ptr<ExecutableNodeObject> thisObjectPtr)
{
	setThisCS_ObjectPtr(thisObjectPtr);
}

bool ExecutableNodeObject::equals(std::shared_ptr<fUML::Semantics::Values::Value> otherValue)
{
	bool isEqual = false;
	
	std::shared_ptr<ExecutableNodeObject> otherExecutableNodeObject = std::dynamic_pointer_cast<ExecutableNodeObject>(otherValue);

	if(otherExecutableNodeObject != nullptr)
	{
		if(this->getExecutableNodeValue() == otherExecutableNodeObject->getExecutableNodeValue())
		{
			isEqual = true;
		}
	}
...
std::string ExecutableNodeObject::toString()
{
	std::string buffer = "Instance of 'ExecutableNodeObject', " + std::to_string(getTypes()->size()) + " types (";
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Object>> ExecutableNodeObject::getDirectContainers()
{   
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Link>> ExecutableNodeObject::getLinks(std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_InteractionPoint> interactionPoint)
{
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> ExecutableNodeObject::retrieveEndValueAsInteractionPoint(std::shared_ptr<fUML::Semantics::Values::Value> endValue, std::shared_ptr<uml::ConnectorEnd> end)
{
	return nullptr;
}

void ExecutableNodeObject::removeValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<fUML::Semantics::Values::Value> value)
{
	...
	if (m_ExecutableNodeValue == nullptr)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": no value stored inside ExecutableNodeObject (property: " << feature->getName() << ")" << std::endl;
		throw "NullPointerException";	
	}

	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity())
	{
				m_ExecutableNodeValue->getActivity().reset();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		...
		{
			m_ExecutableNodeValue->getInInterruptibleRegion()->clear();
		}
		else
		{
			...
			{
				m_ExecutableNodeValue->getInInterruptibleRegion()->erase(inputValue->getInterruptibleActivityRegionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getInPartition()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getInPartition()->erase(inputValue->getActivityPartitionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode())
	{
				m_ExecutableNodeValue->getInStructuredNode().reset();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getIncoming()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getIncoming()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getOutgoing()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getOutgoing()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getRedefinedNode()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getRedefinedNode()->erase(inputValue->getActivityNodeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getOwnedComment()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getOwnedComment()->erase(inputValue->getCommentValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler())
	{
		if (value == nullptr) // clear mode
		{
			m_ExecutableNodeValue->getHandler()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ExecutableNodeValue->getHandler()->erase(inputValue->getExceptionHandlerValue());
			}
		}
	}
	...
	{
				m_ExecutableNodeValue->getNameExpression().reset();
	...
	{
				m_ExecutableNodeValue->setIsLeaf(false /*defined default value*/);

	}
}


std::shared_ptr<Bag<fUML::Semantics::Values::Value>> ExecutableNodeObject::getValues(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> featureValues)
{
	if (feature->getMetaElementID() != uml::UmlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::UmlPackage::PORT_CLASS && feature->getMetaElementID() != uml::UmlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity())
	{
		...
		value->setActivityValue(m_ExecutableNodeValue->getActivity().lock());
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode())
	{
		...
		value->setLocus(this->getLocus());
		value->setStructuredActivityNodeValue(m_ExecutableNodeValue->getInStructuredNode().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> incomingList = m_ExecutableNodeValue->getIncoming();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> outgoingList = m_ExecutableNodeValue->getOutgoing();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_owner())
	{
		...
		value->setThisElementObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setElementValue(m_ExecutableNodeValue->getOwner().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Bag<uml::Dependency>> clientDependencyList = m_ExecutableNodeValue->getClientDependency();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		value->setValue(m_ExecutableNodeValue->getName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		value->setLocus(this->getLocus());
		value->setStringExpressionValue(m_ExecutableNodeValue->getNameExpression());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace())
	{
		...
		value->setLocus(this->getLocus());
		value->setNamespaceValue(m_ExecutableNodeValue->getNamespace().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::StringValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createStringValue();
		value->setValue(m_ExecutableNodeValue->getQualifiedName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		uml::VisibilityKind visibility = m_ExecutableNodeValue->getVisibility();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		value->setValue(m_ExecutableNodeValue->getIsLeaf());
		values->add(value);
	}
...
std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue> ExecutableNodeObject::retrieveFeatureValue(std::shared_ptr<uml::StructuralFeature> feature)
{
...
void ExecutableNodeObject::setFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	if (feature->getMetaElementID() != uml::UmlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::UmlPackage::PORT_CLASS && feature->getMetaElementID() != uml::UmlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getInInterruptibleRegion()->push_back(value->getInterruptibleActivityRegionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getInPartition()->push_back(value->getActivityPartitionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getIncoming()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getOutgoing()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getRedefinedNode()->push_back(value->getActivityNodeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getOwnedComment()->push_back(value->getCommentValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ExecutableNodeValue->getHandler()->push_back(value->getExceptionHandlerValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		{
			m_ExecutableNodeValue->setName(valueObject->getValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		if (value != nullptr)
		{
			m_ExecutableNodeValue->setNameExpression(value->getStringExpressionValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		...
		{
			m_ExecutableNodeValue->setVisibility(uml::VisibilityKind::PUBLIC);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_private())
		{
			m_ExecutableNodeValue->setVisibility(uml::VisibilityKind::PRIVATE);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_protected())
		{
			m_ExecutableNodeValue->setVisibility(uml::VisibilityKind::PROTECTED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_package())
		{
			m_ExecutableNodeValue->setVisibility(uml::VisibilityKind::PACKAGE);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		if (valueObject != nullptr)
		{
			m_ExecutableNodeValue->setIsLeaf(valueObject->isValue());
		}
		
	}
}

void ExecutableNodeObject::assignFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	this->setFeatureValue(feature, values, position);
}

std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> ExecutableNodeObject::retrieveFeatureValues()
{
	...
	for(unsigned int i = 0; i < allAttributesSize; i++)
	{
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity() && m_ExecutableNodeValue->getActivity().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inGroup() && m_ExecutableNodeValue->getInGroup() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion() && m_ExecutableNodeValue->getInInterruptibleRegion() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition() && m_ExecutableNodeValue->getInPartition() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode() && m_ExecutableNodeValue->getInStructuredNode().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming() && m_ExecutableNodeValue->getIncoming() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing() && m_ExecutableNodeValue->getOutgoing() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode() && m_ExecutableNodeValue->getRedefinedNode() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment() && m_ExecutableNodeValue->getOwnedComment() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedElement() && m_ExecutableNodeValue->getOwnedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_owner() && m_ExecutableNodeValue->getOwner().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler() && m_ExecutableNodeValue->getHandler() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_clientDependency() && m_ExecutableNodeValue->getClientDependency() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression() && m_ExecutableNodeValue->getNameExpression() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace() && m_ExecutableNodeValue->getNamespace().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement() && m_ExecutableNodeValue->getRedefinedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinitionContext() && m_ExecutableNodeValue->getRedefinitionContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
	}
