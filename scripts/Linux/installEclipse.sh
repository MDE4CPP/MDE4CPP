...
if [[ "${ARCH}" == "x86_64" ]]; then
  ...
  echo "[installEclipse] ERROR: Unsupported architecture: ${ARCH}"
  exit 1
fi
...
if [[ -x "${TARGET_DIR}/eclipse" ]]; then
  if ! "${TARGET_DIR}/eclipse" -nosplash -consoleLog -application org.eclipse.equinox.p2.director -help >/dev/null 2>&1; then
    echo "Existing Eclipse binary is broken or incompatible. Forcing re-install..."
    rm -rf "${TARGET_DIR}"
  else
...
if ls "${TARGET_DIR}/features/org.eclipse.acceleo_"* >/dev/null 2>&1; then
  echo "Acceleo is already installed. Skipping."
else
...
if ls "${TARGET_DIR}/features/org.eclipse.sirius.aql_"* >/dev/null 2>&1; then
  echo "Sirius is already installed. Skipping."
else
...
if ls "${TARGET_DIR}/features/org.eclipse.cdt_"* >/dev/null 2>&1; then
  echo "CDT is already installed. Skipping."
else
