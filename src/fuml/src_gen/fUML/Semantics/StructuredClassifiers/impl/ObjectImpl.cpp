...
std::shared_ptr<fUML::Semantics::Values::Value> ObjectImpl::new_()
{
	//ADD_COUNT(__PRETTY_FUNCTION__)
	//generated from body annotation
	return std::shared_ptr<fUML::Semantics::Values::Value>(FUMLFactory::eInstance()->createObject());
	//end of body
}
...
void ObjectImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		if (objectActivation != nullptr)
		{
			saveHandler->addReference(objectActivation, "objectActivation", objectActivation->eClass() != package->fUML::FUMLPackage::eInstance()->getObjectActivation_Class());
		}
	}
	catch (std::exception& e)
	{
