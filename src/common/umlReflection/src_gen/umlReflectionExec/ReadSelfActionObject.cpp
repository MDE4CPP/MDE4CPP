#include "umlReflectionExec/ReadSelfActionObject.hpp"
...
{	
	m_ReadSelfActionValue.reset();

	fUML::Semantics::StructuredClassifiers::ObjectImpl::destroy();
}

std::shared_ptr<uml::Element> ReadSelfActionObject::getUmlValue() const
{
	return getReadSelfActionValue();
}

std::shared_ptr<uml::ReadSelfAction> ReadSelfActionObject::getReadSelfActionValue() const
{
	return m_ReadSelfActionValue;
}

void ReadSelfActionObject::setUmlValue(std::shared_ptr<uml::Element> _element)
{
	setReadSelfActionValue(std::dynamic_pointer_cast<uml::ReadSelfAction>(_element));
}

void ReadSelfActionObject::setReadSelfActionValue(std::shared_ptr<uml::ReadSelfAction> _ReadSelfActionElement)
{
	m_ReadSelfActionValue = _ReadSelfActionElement;
	//set super type values
	UML::ActionObject::setActionValue(_ReadSelfActionElement);
}

void ReadSelfActionObject::setThisReadSelfActionObjectPtr(std::weak_ptr<ReadSelfActionObject> thisObjectPtr)
{
	setThisCS_ObjectPtr(thisObjectPtr);
}

bool ReadSelfActionObject::equals(std::shared_ptr<fUML::Semantics::Values::Value> otherValue)
{
	bool isEqual = false;
	
	std::shared_ptr<ReadSelfActionObject> otherReadSelfActionObject = std::dynamic_pointer_cast<ReadSelfActionObject>(otherValue);

	if(otherReadSelfActionObject != nullptr)
	{
		if(this->getReadSelfActionValue() == otherReadSelfActionObject->getReadSelfActionValue())
		{
			isEqual = true;
		}
	}
...
std::string ReadSelfActionObject::toString()
{
	std::string buffer = "Instance of 'ReadSelfActionObject', " + std::to_string(getTypes()->size()) + " types (";
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Object>> ReadSelfActionObject::getDirectContainers()
{   
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Link>> ReadSelfActionObject::getLinks(std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_InteractionPoint> interactionPoint)
{
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> ReadSelfActionObject::retrieveEndValueAsInteractionPoint(std::shared_ptr<fUML::Semantics::Values::Value> endValue, std::shared_ptr<uml::ConnectorEnd> end)
{
	/*Not done for metamodel object classes*/
...
void ReadSelfActionObject::removeValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<fUML::Semantics::Values::Value> value)
{
	...
	if (m_ReadSelfActionValue == nullptr)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": no value stored inside ReadSelfActionObject (property: " << feature->getName() << ")" << std::endl;
		throw "NullPointerException";	
	}

	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_isLocallyReentrant())
	{
				m_ReadSelfActionValue->setIsLocallyReentrant(false /*defined default value*/);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPostcondition())
	{
		...
		{
			m_ReadSelfActionValue->getLocalPostcondition()->clear();
		}
		else
		{
			...
			{
				m_ReadSelfActionValue->getLocalPostcondition()->erase(inputValue->getConstraintValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPrecondition())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getLocalPrecondition()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getLocalPrecondition()->erase(inputValue->getConstraintValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getInInterruptibleRegion()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getInInterruptibleRegion()->erase(inputValue->getInterruptibleActivityRegionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getInPartition()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getInPartition()->erase(inputValue->getActivityPartitionValue());
			}
		}
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getIncoming()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getIncoming()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getOutgoing()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getOutgoing()->erase(inputValue->getActivityEdgeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getRedefinedNode()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getRedefinedNode()->erase(inputValue->getActivityNodeValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getOwnedComment()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getOwnedComment()->erase(inputValue->getCommentValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler())
	{
		if (value == nullptr) // clear mode
		{
			m_ReadSelfActionValue->getHandler()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_ReadSelfActionValue->getHandler()->erase(inputValue->getExceptionHandlerValue());
			}
		}
	}
	...
	{
				m_ReadSelfActionValue->setIsLeaf(false /*defined default value*/);

	}
}


std::shared_ptr<Bag<fUML::Semantics::Values::Value>> ReadSelfActionObject::getValues(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> featureValues)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_context())
	{
		...
		value->setClassifierValue(m_ReadSelfActionValue->getContext());
	...
	{
		std::shared_ptr<SubsetUnion<uml::InputPin, uml::Element>> inputList = m_ReadSelfActionValue->getInput();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_isLocallyReentrant())
	{
		...
		value->setValue(m_ReadSelfActionValue->getIsLocallyReentrant());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPostcondition())
	{
		std::shared_ptr<Subset<uml::Constraint, uml::Element>> localPostconditionList = m_ReadSelfActionValue->getLocalPostcondition();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPrecondition())
	{
		std::shared_ptr<Subset<uml::Constraint, uml::Element>> localPreconditionList = m_ReadSelfActionValue->getLocalPrecondition();
	...
	{
		std::shared_ptr<SubsetUnion<uml::OutputPin, uml::Element>> outputList = m_ReadSelfActionValue->getOutput();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity())
	{
		...
		value->setLocus(this->getLocus());
		value->setActivityValue(m_ReadSelfActionValue->getActivity().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Union<uml::ActivityGroup>> inGroupList = m_ReadSelfActionValue->getInGroup();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		std::shared_ptr<Subset<uml::InterruptibleActivityRegion, uml::ActivityGroup>> inInterruptibleRegionList = m_ReadSelfActionValue->getInInterruptibleRegion();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		std::shared_ptr<Subset<uml::ActivityPartition, uml::ActivityGroup>> inPartitionList = m_ReadSelfActionValue->getInPartition();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode())
	{
		...
		value->setLocus(this->getLocus());
		value->setStructuredActivityNodeValue(m_ReadSelfActionValue->getInStructuredNode().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> incomingList = m_ReadSelfActionValue->getIncoming();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		std::shared_ptr<Bag<uml::ActivityEdge>> outgoingList = m_ReadSelfActionValue->getOutgoing();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		std::shared_ptr<Subset<uml::ActivityNode, uml::RedefinableElement>> redefinedNodeList = m_ReadSelfActionValue->getRedefinedNode();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		std::shared_ptr<Subset<uml::Comment, uml::Element>> ownedCommentList = m_ReadSelfActionValue->getOwnedComment();
	...
	{
		std::shared_ptr<Union<uml::Element>> ownedElementList = m_ReadSelfActionValue->getOwnedElement();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_owner())
	{
		...
		value->setThisElementObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setElementValue(m_ReadSelfActionValue->getOwner().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler())
	{
		std::shared_ptr<Subset<uml::ExceptionHandler, uml::Element>> handlerList = m_ReadSelfActionValue->getHandler();
	...
	{
		std::shared_ptr<Bag<uml::Dependency>> clientDependencyList = m_ReadSelfActionValue->getClientDependency();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		value->setValue(m_ReadSelfActionValue->getName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		value->setLocus(this->getLocus());
		value->setStringExpressionValue(m_ReadSelfActionValue->getNameExpression());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace())
	{
		...
		value->setLocus(this->getLocus());
		value->setNamespaceValue(m_ReadSelfActionValue->getNamespace().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::StringValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createStringValue();
		value->setValue(m_ReadSelfActionValue->getQualifiedName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		uml::VisibilityKind visibility = m_ReadSelfActionValue->getVisibility();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ReadSelfAction_result())
	{
		...
		value->setThisOutputPinObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setOutputPinValue(m_ReadSelfActionValue->getResult());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_ReadSelfActionValue->getIsLeaf());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement())
	{
		std::shared_ptr<Union<uml::RedefinableElement>> redefinedElementList = m_ReadSelfActionValue->getRedefinedElement();
	...
	{
		std::shared_ptr<Union<uml::Classifier>> redefinitionContextList = m_ReadSelfActionValue->getRedefinitionContext();
...
std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue> ReadSelfActionObject::retrieveFeatureValue(std::shared_ptr<uml::StructuralFeature> feature)
{
...
void ReadSelfActionObject::setFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	if (feature->getMetaElementID() != uml::umlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::umlPackage::PORT_CLASS && feature->getMetaElementID() != uml::umlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_isLocallyReentrant())
	{
		...
		{
			m_ReadSelfActionValue->setIsLocallyReentrant(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPostcondition())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getLocalPostcondition()->push_back(value->getConstraintValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Action_localPrecondition())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getLocalPrecondition()->push_back(value->getConstraintValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getInInterruptibleRegion()->push_back(value->getInterruptibleActivityRegionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getInPartition()->push_back(value->getActivityPartitionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getIncoming()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getOutgoing()->push_back(value->getActivityEdgeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getRedefinedNode()->push_back(value->getActivityNodeValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getOwnedComment()->push_back(value->getCommentValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_ReadSelfActionValue->getHandler()->push_back(value->getExceptionHandlerValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		if (valueObject != nullptr)
		{
			m_ReadSelfActionValue->setName(valueObject->getValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		if (value != nullptr)
		{
			m_ReadSelfActionValue->setNameExpression(value->getStringExpressionValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		...
		{
			m_ReadSelfActionValue->setVisibility(uml::VisibilityKind::PUBLIC);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_private())
		{
			m_ReadSelfActionValue->setVisibility(uml::VisibilityKind::PRIVATE);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_protected())
		{
			m_ReadSelfActionValue->setVisibility(uml::VisibilityKind::PROTECTED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_package())
		{
			m_ReadSelfActionValue->setVisibility(uml::VisibilityKind::PACKAGE);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_ReadSelfAction_result())
	{
		...
		if (value != nullptr)
		{
			m_ReadSelfActionValue->setResult(value->getOutputPinValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		if (valueObject != nullptr)
		{
			m_ReadSelfActionValue->setIsLeaf(valueObject->isValue());
		}
		
	}
}

void ReadSelfActionObject::assignFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	this->setFeatureValue(feature, values, position);
}

std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> ReadSelfActionObject::retrieveFeatureValues()
{
	...
	for(unsigned int i = 0; i < allAttributesSize; i++)
	{
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_Action_context() && m_ReadSelfActionValue->getContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Action_input() && m_ReadSelfActionValue->getInput() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Action_localPostcondition() && m_ReadSelfActionValue->getLocalPostcondition() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Action_localPrecondition() && m_ReadSelfActionValue->getLocalPrecondition() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Action_output() && m_ReadSelfActionValue->getOutput() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_activity() && m_ReadSelfActionValue->getActivity().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inGroup() && m_ReadSelfActionValue->getInGroup() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inInterruptibleRegion() && m_ReadSelfActionValue->getInInterruptibleRegion() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inPartition() && m_ReadSelfActionValue->getInPartition() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_inStructuredNode() && m_ReadSelfActionValue->getInStructuredNode().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_incoming() && m_ReadSelfActionValue->getIncoming() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_outgoing() && m_ReadSelfActionValue->getOutgoing() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ActivityNode_redefinedNode() && m_ReadSelfActionValue->getRedefinedNode() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment() && m_ReadSelfActionValue->getOwnedComment() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedElement() && m_ReadSelfActionValue->getOwnedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_owner() && m_ReadSelfActionValue->getOwner().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ExecutableNode_handler() && m_ReadSelfActionValue->getHandler() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_clientDependency() && m_ReadSelfActionValue->getClientDependency() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression() && m_ReadSelfActionValue->getNameExpression() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace() && m_ReadSelfActionValue->getNamespace().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_ReadSelfAction_result() && m_ReadSelfActionValue->getResult() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinedElement() && m_ReadSelfActionValue->getRedefinedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_redefinitionContext() && m_ReadSelfActionValue->getRedefinitionContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
	}
