...
void ActionActivationImpl::createNodeActivations()
{
	...
	const std::shared_ptr<Bag<uml::InputPin>>& inputPins = action->getInput();
    	for(const std::shared_ptr<uml::InputPin>& pin : *inputPins)
    	{
    		...
    		{
			DEBUG_MESSAGE(std::cout<<"An input pin was nullptr!"<<std::endl;)
...
//DEBUG_INFO("Action '" << this->getNode()->getName() << "' retrieving tokens from input pin '" << pin->getName()<<"'.")
    ...
    for(const std::shared_ptr<fUML::Semantics::Activities::Token>& token : *tokenList)
    {
        ...
        if(objToken!=nullptr)
        {
		const std::shared_ptr<fUML::Semantics::Values::Value>& value = objToken->getValue();
		if(value != nullptr)
            {
...
bool ActionActivationImpl::isReady()
{
    ...
    if(actionNode != nullptr)
    {
        ready = fUML::Semantics::Activities::ActivityNodeActivationImpl::isReady()
         ...
         if(ready)
         {
        	const std::shared_ptr<Subset<fUML::Semantics::Actions::InputPinActivation, fUML::Semantics::Actions::PinActivation >>& activations = this->getInputPinActivation();
...
bool ActionActivationImpl::valueParticipatesInLink(const std::shared_ptr<fUML::Semantics::Values::Value>& value, const std::shared_ptr<fUML::Semantics::StructuredClassifiers::Link>& link)
{
	...
	const std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>>& featureValueList = link->getFeatureValues();
    for (const std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue>& featureValue : *featureValueList)
    {
