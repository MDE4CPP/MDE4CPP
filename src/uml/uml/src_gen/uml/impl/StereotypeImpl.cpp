...
void StereotypeImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("icon") == 0 )
		{
			...
			std::shared_ptr<uml::Image> new_icon = std::dynamic_pointer_cast<uml::Image>(modelFactory->create(typeName, loadHandler->getCurrentObject(), uml::umlPackage::STEREOTYPE_ATTRIBUTE_ICON));
			if(new_icon)
			{
				loadHandler->handleChild(new_icon);
				getIcon()->push_back(new_icon);
			} 
