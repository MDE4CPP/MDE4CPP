...
TupleLiteralExpImpl& TupleLiteralExpImpl::operator=(const TupleLiteralExpImpl & obj)
{
	...
	const std::shared_ptr<Bag<ocl::Expressions::TupleLiteralPart>>& partList = obj.getPart();
	if(partList)
	{
		...
		{
			const std::shared_ptr<ocl::Expressions::TupleLiteralPart>& temp = std::dynamic_pointer_cast<ocl::Expressions::TupleLiteralPart>((partindexElem)->copy());
		...
		case ocl::Expressions::ExpressionsPackage::TUPLELITERALEXP_ATTRIBUTE_PART:
		{
			...
			{ 
				try
				{
					...
					const std::shared_ptr<Bag<ocl::Expressions::TupleLiteralPart>>& _part=getPart();
