...
bool AbstractionImpl::eSet(int featureID, std::shared_ptr<Any> newValue)
{
	switch(featureID)
	{
		case uml::umlPackage::ABSTRACTION_ATTRIBUTE_MAPPING:
		{
			...
			if(ecoreAny)
			{
				...
				{
					DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid type stored in 'ecore::ecoreAny' for feature 'mapping'. Failed to set feature!"<< std::endl;)
					return false;
				}
			}
			else
			{
				DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid instance of 'ecore::ecoreAny' for feature 'mapping'. Failed to set feature!"<< std::endl;)
				return false;
			}
		return true;
		}
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> AbstractionImpl::eInvoke(int operationID, std::shared_ptr<Bag<Any>> arguments)
{
