...
void printObject(std::shared_ptr<ecore::EObject> uobj);
void printUmlAny(Any value, bool isMany);
...
void ecore_any_query3() {
    ...
    printEObject(value->get<std::shared_ptr<ecore::EObject>>());
...
void printUmlAny(Any value, bool isMany) {
    if(value != nullptr && !value->isEmpty()) {
        if(isMany) {
            std::shared_ptr<Bag<ecore::EObject>> valueItems = value->get<std::shared_ptr<Bag<ecore::EObject>>>();
            for(size_t i = 0; i < valueItems->size(); i++) {
                printObject(valueItems->at(i));
            }
        }
        else {
            try {
                std::shared_ptr<uml::Object> obj = value->get<std::shared_ptr<uml::Object>>();
