...
{	
    ...
    std::shared_ptr<CalcModel::PrimeCheckerExecution> ActInputNodeUmlLinker(new CalcModel::PrimeCheckerExecution());
    ...
    std::shared_ptr<std::vector<std::shared_ptr<fUML::ParameterValue>>> resultPList = locus->getExecutor()->execute(CalcModel::CalcModelPackage::eInstance()->get_CalcModel_CheckIfPrime(),nullptr,pList);
