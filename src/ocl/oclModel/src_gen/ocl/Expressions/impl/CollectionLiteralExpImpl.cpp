...
CollectionLiteralExpImpl& CollectionLiteralExpImpl::operator=(const CollectionLiteralExpImpl & obj)
{
	...
	const std::shared_ptr<Bag<ocl::Expressions::CollectionLiteralPart>>& partList = obj.getPart();
	if(partList)
	{
		...
		{
			const std::shared_ptr<ocl::Expressions::CollectionLiteralPart>& temp = std::dynamic_pointer_cast<ocl::Expressions::CollectionLiteralPart>((partindexElem)->copy());
...
void CollectionLiteralExpImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("part") == 0 )
		{
			...
			std::shared_ptr<ocl::Expressions::CollectionLiteralPart> new_part = std::dynamic_pointer_cast<ocl::Expressions::CollectionLiteralPart>(modelFactory->create(typeName, loadHandler->getCurrentObject(), ocl::Expressions::ExpressionsPackage::COLLECTIONLITERALEXP_ATTRIBUTE_PART));
			if(new_part)
			{
				loadHandler->handleChild(new_part);
				getPart()->push_back(new_part);
			} 
...
bool CollectionLiteralExpImpl::eSet(int featureID,  const std::shared_ptr<Any>& newValue)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::COLLECTIONLITERALEXP_ATTRIBUTE_KIND:
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> CollectionLiteralExpImpl::eInvoke(int operationID, const std::shared_ptr<Bag<Any>>& arguments)
{
