...
// Operations
//*********************************
bool PackageableElementImpl::namespace_needs_visibility(std::shared_ptr<Any> diagnostics, std::shared_ptr<std::map < Any, Any>> context)
{
	...
	switch(featureID)
	{
		case uml::umlPackage::PACKAGEABLEELEMENT_ATTRIBUTE_OWNINGPACKAGE:
		{
			...
			if(ecoreAny)
			{
				...
				{
					DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid type stored in 'ecore::ecoreAny' for feature 'owningPackage'. Failed to set feature!"<< std::endl;)
					return false;
				}
			}
			else
			{
				DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid instance of 'ecore::ecoreAny' for feature 'owningPackage'. Failed to set feature!"<< std::endl;)
				return false;
			}
		return true;
		}
...
std::shared_ptr<Any> PackageableElementImpl::eInvoke(int operationID, std::shared_ptr<Bag<Any>> arguments)
{
	std::shared_ptr<Any> result;
  	...
  	switch(operationID)
	{
		// uml::PackageableElement::namespace_needs_visibility(Any, std::map) : bool: 369327334
