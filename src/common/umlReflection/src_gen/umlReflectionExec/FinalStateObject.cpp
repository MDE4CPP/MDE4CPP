#include "umlReflectionExec/FinalStateObject.hpp"
...
{	
	if(m_FinalStateValue)
	{

	}

	m_FinalStateValue.reset();

	fUML::Semantics::StructuredClassifiers::ObjectImpl::destroy();
}

std::shared_ptr<uml::Element> FinalStateObject::getUmlValue() const
{
	return getFinalStateValue();
}

std::shared_ptr<uml::FinalState> FinalStateObject::getFinalStateValue() const
{
	return m_FinalStateValue;
}

void FinalStateObject::setUmlValue(std::shared_ptr<uml::Element> _element)
{
	setFinalStateValue(std::dynamic_pointer_cast<uml::FinalState>(_element));
}

void FinalStateObject::setFinalStateValue(std::shared_ptr<uml::FinalState> _FinalStateElement)
{
	m_FinalStateValue = _FinalStateElement;
	//set super type values
	UML::StateObject::setStateValue(_FinalStateElement);
}

void FinalStateObject::setThisFinalStateObjectPtr(std::weak_ptr<FinalStateObject> thisObjectPtr)
{
	setThisCS_ObjectPtr(thisObjectPtr);
}

bool FinalStateObject::equals(std::shared_ptr<fUML::Semantics::Values::Value> otherValue)
{
	bool isEqual = false;
	
	std::shared_ptr<FinalStateObject> otherFinalStateObject = std::dynamic_pointer_cast<FinalStateObject>(otherValue);

	if(otherFinalStateObject != nullptr)
	{
		if(this->getFinalStateValue() == otherFinalStateObject->getFinalStateValue())
		{
			isEqual = true;
		}
	}
...
std::string FinalStateObject::toString()
{
	std::string buffer = "Instance of 'FinalStateObject', " + std::to_string(getTypes()->size()) + " types (";
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Object>> FinalStateObject::getDirectContainers()
{   
...
std::shared_ptr<Bag<PSCS::Semantics::StructuredClassifiers::CS_Link>> FinalStateObject::getLinks(std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_InteractionPoint> interactionPoint)
{
...
std::shared_ptr<Bag<fUML::Semantics::Values::Value>> FinalStateObject::retrieveEndValueAsInteractionPoint(std::shared_ptr<fUML::Semantics::Values::Value> endValue, std::shared_ptr<uml::ConnectorEnd> end)
{
	return nullptr;
}

void FinalStateObject::removeValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<fUML::Semantics::Values::Value> value)
{
	...
	if (m_FinalStateValue == nullptr)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": no value stored inside FinalStateObject (property: " << feature->getName() << ")" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		{
			m_FinalStateValue->getOwnedComment()->clear();
		}
		else
		{
			...
			{
				m_FinalStateValue->getOwnedComment()->erase(inputValue->getCommentValue());
			}
		}
	}
	...
	{
				m_FinalStateValue->getNameExpression().reset();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getElementImport()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getElementImport()->erase(inputValue->getElementImportValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getOwnedRule()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getOwnedRule()->erase(inputValue->getConstraintValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getPackageImport()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getPackageImport()->erase(inputValue->getPackageImportValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
				m_FinalStateValue->setIsLeaf(false /*defined default value*/);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_connection())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getConnection()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getConnection()->erase(inputValue->getConnectionPointReferenceValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_connectionPoint())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getConnectionPoint()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getConnectionPoint()->erase(inputValue->getPseudostateValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_deferrableTrigger())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getDeferrableTrigger()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_region())
	{
		if (value == nullptr) // clear mode
		{
			m_FinalStateValue->getRegion()->clear();
		}
		else
		{
			/* Should use PSCS::CS_Reference but dynamic_pointer_cast fails --> using fUML::Reference instead
			...
			if (inputValue != nullptr)
			{
				m_FinalStateValue->getRegion()->erase(inputValue->getRegionValue());
			}
		}
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_stateInvariant())
	{
				m_FinalStateValue->getStateInvariant().reset();

	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_submachine())
	{
				m_FinalStateValue->getSubmachine().reset();

	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Vertex_container())
	{
				m_FinalStateValue->getContainer().reset();

	}
}


std::shared_ptr<Bag<fUML::Semantics::Values::Value>> FinalStateObject::getValues(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> featureValues)
{
	if (feature->getMetaElementID() != uml::UmlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::UmlPackage::PORT_CLASS && feature->getMetaElementID() != uml::UmlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_owner())
	{
		...
		value->setThisElementObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setElementValue(m_FinalStateValue->getOwner().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Bag<uml::Dependency>> clientDependencyList = m_FinalStateValue->getClientDependency();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		value->setValue(m_FinalStateValue->getName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		value->setLocus(this->getLocus());
		value->setStringExpressionValue(m_FinalStateValue->getNameExpression());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace())
	{
		...
		value->setLocus(this->getLocus());
		value->setNamespaceValue(m_FinalStateValue->getNamespace().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::StringValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createStringValue();
		value->setValue(m_FinalStateValue->getQualifiedName());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		uml::VisibilityKind visibility = m_FinalStateValue->getVisibility();
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		value->setValue(m_FinalStateValue->getIsLeaf());
		values->add(value);
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_doActivity())
	{
		...
		value->setLocus(this->getLocus());
		value->setBehaviorValue(m_FinalStateValue->getDoActivity());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_entry())
	{
		...
		value->setThisBehaviorObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setBehaviorValue(m_FinalStateValue->getEntry());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_exit())
	{
		...
		value->setThisBehaviorObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setBehaviorValue(m_FinalStateValue->getExit());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_FinalStateValue->getIsComposite());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_isOrthogonal())
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_FinalStateValue->getIsOrthogonal());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_isSimple())
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_FinalStateValue->getIsSimple());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_isSubmachineState())
	{
		std::shared_ptr<fUML::Semantics::SimpleClassifiers::BooleanValue> value = fUML::Semantics::SimpleClassifiers::SimpleClassifiersFactory::eInstance()->createBooleanValue();
		value->setValue(m_FinalStateValue->getIsSubmachineState());
		values->add(value);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_redefinedState())
	{
		...
		value->setLocus(this->getLocus());
		value->setStateValue(m_FinalStateValue->getRedefinedState());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_stateInvariant())
	{
		...
		value->setThisConstraintObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setConstraintValue(m_FinalStateValue->getStateInvariant());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		reference->setCompositeReferent(value);
		values->add(reference);
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_submachine())
	{
		...
		value->setLocus(this->getLocus());
		value->setStateMachineValue(m_FinalStateValue->getSubmachine());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	if (feature == UML::UMLPackage::eInstance()->get_UML_Vertex_container())
	{
		...
		value->setThisRegionObjectPtr(value);
		value->setLocus(this->getLocus());
		value->setRegionValue(m_FinalStateValue->getContainer().lock());
		std::shared_ptr<PSCS::Semantics::StructuredClassifiers::CS_Reference> reference = PSCS::Semantics::StructuredClassifiers::StructuredClassifiersFactory::eInstance()->createCS_Reference();
		reference->setReferent(value);
		values->add(reference);
	}
	...
	{
		std::shared_ptr<Bag<uml::Transition>> incomingList = m_FinalStateValue->getIncoming();
	...
	{
		std::shared_ptr<Bag<uml::Transition>> outgoingList = m_FinalStateValue->getOutgoing();
...
std::shared_ptr<fUML::Semantics::SimpleClassifiers::FeatureValue> FinalStateObject::retrieveFeatureValue(std::shared_ptr<uml::StructuralFeature> feature)
{
...
void FinalStateObject::setFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	if (feature->getMetaElementID() != uml::UmlPackage::PROPERTY_CLASS && feature->getMetaElementID() != uml::UmlPackage::PORT_CLASS && feature->getMetaElementID() != uml::UmlPackage::EXTENSIONEND_CLASS)
	{
		std::cerr << __PRETTY_FUNCTION__ << ": feature is null or not kind of uml::Property" << std::endl;
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getOwnedComment()->push_back(value->getCommentValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_name())
	{
		...
		{
			m_FinalStateValue->setName(valueObject->getValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setNameExpression(value->getStringExpressionValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_NamedElement_visibility())
	{
		...
		{
			m_FinalStateValue->setVisibility(uml::VisibilityKind::PUBLIC);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_private())
		{
			m_FinalStateValue->setVisibility(uml::VisibilityKind::PRIVATE);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_protected())
		{
			m_FinalStateValue->setVisibility(uml::VisibilityKind::PROTECTED);
		}
		if (literal == UML::UMLPackage::eInstance()->get_UML_VisibilityKind_package())
		{
			m_FinalStateValue->setVisibility(uml::VisibilityKind::PACKAGE);
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getElementImport()->push_back(value->getElementImportValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getOwnedRule()->push_back(value->getConstraintValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getPackageImport()->push_back(value->getPackageImportValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_RedefinableElement_isLeaf())
	{
		...
		if (valueObject != nullptr)
		{
			m_FinalStateValue->setIsLeaf(valueObject->isValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_connection())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getConnection()->push_back(value->getConnectionPointReferenceValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_connectionPoint())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getConnectionPoint()->push_back(value->getPseudostateValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_deferrableTrigger())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getDeferrableTrigger()->push_back(value->getTriggerValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_doActivity())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setDoActivity(value->getBehaviorValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_entry())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setEntry(value->getBehaviorValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_exit())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setExit(value->getBehaviorValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_redefinedState())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setRedefinedState(value->getStateValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_region())
	{
		...
		while (iter != end)
		{
			...
			if (value != nullptr)
			{
				m_FinalStateValue->getRegion()->push_back(value->getRegionValue());
			}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_stateInvariant())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setStateInvariant(value->getConstraintValue());
		}
	...
	if (feature == UML::UMLPackage::eInstance()->get_UML_State_submachine())
	{
		...
		if (value != nullptr)
		{
			m_FinalStateValue->setSubmachine(value->getStateMachineValue());
		}
		
	}
}

void FinalStateObject::assignFeatureValue(std::shared_ptr<uml::StructuralFeature> feature, std::shared_ptr<Bag<fUML::Semantics::Values::Value>> values, int position)
{
	this->setFeatureValue(feature, values, position);
}

std::shared_ptr<Bag<fUML::Semantics::SimpleClassifiers::FeatureValue>> FinalStateObject::retrieveFeatureValues()
{
	...
	for(unsigned int i = 0; i < allAttributesSize; i++)
	{
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedComment() && m_FinalStateValue->getOwnedComment() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_ownedElement() && m_FinalStateValue->getOwnedElement() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Element_owner() && m_FinalStateValue->getOwner().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_clientDependency() && m_FinalStateValue->getClientDependency() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_nameExpression() && m_FinalStateValue->getNameExpression() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_NamedElement_namespace() && m_FinalStateValue->getNamespace().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_elementImport() && m_FinalStateValue->getElementImport() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_importedMember() && m_FinalStateValue->getImportedMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_member() && m_FinalStateValue->getMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedMember() && m_FinalStateValue->getOwnedMember() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_ownedRule() && m_FinalStateValue->getOwnedRule() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Namespace_packageImport() && m_FinalStateValue->getPackageImport() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		...
		if (property == UML::UMLPackage::eInstance()->get_UML_State_connection() && m_FinalStateValue->getConnection() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_connectionPoint() && m_FinalStateValue->getConnectionPoint() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_deferrableTrigger() && m_FinalStateValue->getDeferrableTrigger() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_doActivity() && m_FinalStateValue->getDoActivity() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_entry() && m_FinalStateValue->getEntry() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_exit() && m_FinalStateValue->getExit() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_redefinedState() && m_FinalStateValue->getRedefinedState() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_redefinitionContext() && m_FinalStateValue->getRedefinitionContext() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_region() && m_FinalStateValue->getRegion() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_stateInvariant() && m_FinalStateValue->getStateInvariant() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_State_submachine() && m_FinalStateValue->getSubmachine() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Vertex_container() && m_FinalStateValue->getContainer().lock() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Vertex_incoming() && m_FinalStateValue->getIncoming() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
		if (property == UML::UMLPackage::eInstance()->get_UML_Vertex_outgoing() && m_FinalStateValue->getOutgoing() != nullptr)
		{
			featureValues->add(this->retrieveFeatureValue(property));
		}
	}
