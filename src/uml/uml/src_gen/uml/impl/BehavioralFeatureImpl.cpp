...
:BehavioralFeatureImpl()
{
	// explain why warning may be used uninitialized [-Wmaybe-uninitialized]
	m_owner = par_owner;
}
...
void BehavioralFeatureImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("ownedParameter") == 0 )
		{
			...
			std::shared_ptr<uml::Parameter> new_ownedParameter = std::dynamic_pointer_cast<uml::Parameter>(modelFactory->create(typeName, loadHandler->getCurrentObject(), uml::umlPackage::BEHAVIORALFEATURE_ATTRIBUTE_OWNEDPARAMETER));
		...
		if ( nodeName.compare("ownedParameterSet") == 0 )
		{
			...
			std::shared_ptr<ecore::ecoreFactory> modelFactory = ecore::ecoreFactory::eInstance();		
			std::shared_ptr<uml::ParameterSet> new_ownedParameterSet = std::dynamic_pointer_cast<uml::ParameterSet>(modelFactory->create(typeName, loadHandler->getCurrentObject(), uml::umlPackage::BEHAVIORALFEATURE_ATTRIBUTE_OWNEDPARAMETERSET));
