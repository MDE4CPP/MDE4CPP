...
{
	std::shared_ptr<ecore::EObject> element(new TestIdentityActionImpl(*this));
	return element;
}
...
void TestIdentityActionImpl::eSet(int featureID, boost::any newValue)
{
	switch(featureID)
	{
		case UmlPackage::ACTIVITYNODE_ACTIVITY:
		...
		case UmlPackage::ACTIVITYNODE_INSTRUCTUREDNODE:
		{
			...
			setInStructuredNode(_inStructuredNode); //17716
			break;
		}
		case UmlPackage::REDEFINABLEELEMENT_ISLEAF:
		{
		...
		case UmlPackage::ACTION_ISLOCALLYREENTRANT:
		{
			...
			setIsLocallyReentrant(_isLocallyReentrant); //17724
			break;
		}
		case UmlPackage::NAMEDELEMENT_NAME:
		{
		...
		case UmlPackage::NAMEDELEMENT_NAMEEXPRESSION:
		{
			...
			setNameExpression(_nameExpression); //1776
			break;
		}
		case UmlPackage::TESTIDENTITYACTION_RESULT:
		{
		...
		case UmlPackage::NAMEDELEMENT_VISIBILITY:
		{
			...
			setVisibility(_visibility); //1779
			break;
		}
	}
}
