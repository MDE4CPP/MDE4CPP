...
#include "fUML/CallEventExecution.hpp"
...
{
	return FUMLPackageImpl::eInstance()->getCallEventOccurrence_Class();
}
...
std::shared_ptr<Bag<fUML::ParameterValue> > CallEventOccurrenceImpl::getParameterValues()
{
	//ADD_COUNT(__PRETTY_FUNCTION__)
	//generated from body annotation
...
void CallEventOccurrenceImpl::setOutputParameterValues(std::shared_ptr<Bag<fUML::ParameterValue> >  parameterValues)
{
	//ADD_COUNT(__PRETTY_FUNCTION__)
	//generated from body annotation
...
// References
//*********************************
std::shared_ptr<fUML::CallEventExecution > CallEventOccurrenceImpl::getExecution() const
{
//assert(m_execution);
    return m_execution;
}
void CallEventOccurrenceImpl::setExecution(std::shared_ptr<fUML::CallEventExecution> _execution)
{
...
Any CallEventOccurrenceImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case FUMLPackage::CALLEVENTOCCURRENCE_ATTRIBUTE_EXECUTION:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getExecution())); //160
	}
...
bool CallEventOccurrenceImpl::internalEIsSet(int featureID) const
{
	...
	{
		case FUMLPackage::CALLEVENTOCCURRENCE_ATTRIBUTE_EXECUTION:
			return getExecution() != nullptr; //160
	}
...
bool CallEventOccurrenceImpl::eSet(int featureID, Any newValue)
{
	switch(featureID)
	{
		case FUMLPackage::CALLEVENTOCCURRENCE_ATTRIBUTE_EXECUTION:
		{
			...
			std::shared_ptr<fUML::CallEventExecution> _execution = std::dynamic_pointer_cast<fUML::CallEventExecution>(_temp);
			setExecution(_execution); //160
			return true;
		}
	}
...
void CallEventOccurrenceImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler, std::shared_ptr<fUML::FUMLFactory> modelFactory)
{
	...
	switch(featureID)
	{
		case FUMLPackage::CALLEVENTOCCURRENCE_ATTRIBUTE_EXECUTION:
		{
			if (references.size() == 1)
			{
				...
				std::shared_ptr<fUML::CallEventExecution> _execution = std::dynamic_pointer_cast<fUML::CallEventExecution>( references.front() );
