...
//Additional constructor for the containments back reference
LiteralExpImpl::LiteralExpImpl(std::weak_ptr<ocl::Expressions::LoopExp> par_LoopExp, const int reference_id)
:LiteralExpImpl()
{
	switch(reference_id)
	{	
...
LiteralExpImpl& LiteralExpImpl::operator=(const LiteralExpImpl & obj)
{
	 ...
	 * Currently, this causes an error because it calls an implicit assignment operator of LiteralExp 
	 * which is generated by the compiler (as LiteralExp is an abstract class and does not have a user-defined assignment operator).
...
//*********************************
std::shared_ptr<Any> LiteralExpImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
	}
...
bool LiteralExpImpl::eSet(int featureID,  const std::shared_ptr<Any>& newValue)
{
	switch(featureID)
	{
	}
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> LiteralExpImpl::eInvoke(int operationID, const std::shared_ptr<Bag<Any>>& arguments)
{
