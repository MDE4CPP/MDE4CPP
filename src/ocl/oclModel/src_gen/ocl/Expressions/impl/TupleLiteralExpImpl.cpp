...
void TupleLiteralExpImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("part") == 0 )
		{
			...
			std::shared_ptr<ocl::Expressions::VarDeclarationExp> new_part = std::dynamic_pointer_cast<ocl::Expressions::VarDeclarationExp>(modelFactory->create(typeName, loadHandler->getCurrentObject(), ocl::Expressions::ExpressionsPackage::TUPLELITERALEXP_ATTRIBUTE_PART));
			if(new_part)
			{
