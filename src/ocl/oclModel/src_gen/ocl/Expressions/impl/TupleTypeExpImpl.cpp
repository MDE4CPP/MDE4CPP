...
void TupleTypeExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	...
	// Add references
	if ( this->eIsSet(package->getTupleTypeExp_Attribute_part()) )
	{
...
bool TupleTypeExpImpl::eSet(int featureID,  const std::shared_ptr<Any>& newValue)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::TUPLETYPEEXP_ATTRIBUTE_PART:
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> TupleTypeExpImpl::eInvoke(int operationID, const std::shared_ptr<Bag<Any>>& arguments)
{
