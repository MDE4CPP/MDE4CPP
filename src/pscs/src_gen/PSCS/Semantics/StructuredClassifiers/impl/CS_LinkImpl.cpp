...
CS_LinkImpl& CS_LinkImpl::operator=(const CS_LinkImpl & obj)
{
	 ...
	 * Currently, this causes an error because it calls an implicit assignment operator of CS_Link 
	 * which is generated by the compiler (as CS_Link is an abstract class and does not have a user-defined assignment operator).
