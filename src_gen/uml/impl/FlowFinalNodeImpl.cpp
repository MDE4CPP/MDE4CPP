...
{
	std::shared_ptr<ecore::EObject> element(new FlowFinalNodeImpl(*this));
	return element;
}
...
void FlowFinalNodeImpl::eSet(int featureID, boost::any newValue)
{
	switch(featureID)
	{
		case UmlPackage::ACTIVITYNODE_ACTIVITY:
		...
		case UmlPackage::ACTIVITYNODE_INSTRUCTUREDNODE:
		{
			...
			setInStructuredNode(_inStructuredNode); //18916
			break;
		}
		case UmlPackage::REDEFINABLEELEMENT_ISLEAF:
		{
		...
		case UmlPackage::NAMEDELEMENT_NAMEEXPRESSION:
		{
			...
			setNameExpression(_nameExpression); //1896
			break;
		}
		case UmlPackage::NAMEDELEMENT_VISIBILITY:
		{
			...
			setVisibility(_visibility); //1899
			break;
		}
	}
}
