...
//Additional constructor for the containments back reference
InfixedExpImpl::InfixedExpImpl(std::weak_ptr<ocl::Expressions::LoopExp> par_LoopExp, const int reference_id)
:InfixedExpImpl()
{
	switch(reference_id)
	{	
...
InfixedExpImpl& InfixedExpImpl::operator=(const InfixedExpImpl & obj)
{
	 ...
	 * Currently, this causes an error because it calls an implicit assignment operator of InfixedExp 
	 * which is generated by the compiler (as InfixedExp is an abstract class and does not have a user-defined assignment operator).
...
void InfixedExpImpl::setSource(const std::shared_ptr<ocl::Expressions::OclExpression>& _source)
{
    m_source = _source;
...
void InfixedExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	...
	// Add references
	if ( this->eIsSet(package->getInfixedExp_Attribute_source()) )
	{
...
//*********************************
std::shared_ptr<Any> InfixedExpImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::INFIXEDEXP_ATTRIBUTE_SOURCE:
			return eAny(getSource(),ocl::Expressions::ExpressionsPackage::OCLEXPRESSION_CLASS,false); //3123
	}
...
bool InfixedExpImpl::internalEIsSet(int featureID) const
{
	...
	{
		case ocl::Expressions::ExpressionsPackage::INFIXEDEXP_ATTRIBUTE_SOURCE:
			return getSource() != nullptr; //3123
	}
	return OclExpressionImpl::internalEIsSet(featureID);
}

bool InfixedExpImpl::eSet(int featureID,  const std::shared_ptr<Any>& newValue)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::INFIXEDEXP_ATTRIBUTE_SOURCE:
		{
			...
			if(ecoreAny)
			{
				try
				{
					...
					std::shared_ptr<ocl::Expressions::OclExpression> _source = std::dynamic_pointer_cast<ocl::Expressions::OclExpression>(eObject);
					if(_source)
					{
						setSource(_source); //3123
					}
					else
					{
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> InfixedExpImpl::eInvoke(int operationID, const std::shared_ptr<Bag<Any>>& arguments)
{
