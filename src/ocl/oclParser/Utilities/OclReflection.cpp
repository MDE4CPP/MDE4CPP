...
Any OclReflection::retrieveRawValue(std::shared_ptr<fUML::Semantics::Values::Value> fromValue)
{
    if(instanceOf<CollectionValue>(fromValue)) {
        ...
        std::shared_ptr<Bag<ecore::EObject>> bagValue = std::make_shared<Bag<ecore::EObject>>();
        ...
        for(size_t i = 0; i < colValue->getElements()->size(); i++) {
            ...
            std::shared_ptr<ObjectValue> objValue = std::dynamic_pointer_cast<ObjectValue>(src);
            if(objValue != nullptr) {
                bagValue->add(objValue->getValue());
            }
        }
