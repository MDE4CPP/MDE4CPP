...
SubsetUnionBenchmarkFactoryImpl::SubsetUnionBenchmarkFactoryImpl()
{
	...
	m_idMap.insert(std::pair<std::string,unsigned int>("ExampleModel", SubsetUnionBenchmarkPackage::EXAMPLEMODEL));
}
...
std::shared_ptr<ecore::EObject> SubsetUnionBenchmarkFactoryImpl::create(const unsigned int classID,  std::shared_ptr<ecore::EObject> container /*= nullptr*/, const unsigned int referenceID/* = -1*/) const
{
	switch(classID)
	{
		...
		case SubsetUnionBenchmarkPackage::EXAMPLEMODEL:
		{
...
std::shared_ptr<ExampleModel> SubsetUnionBenchmarkFactoryImpl::createExampleModel() const
{
