...
ActivityImpl::ActivityImpl()
{
	...
	//*********************************
		/*Subset*/
		m_edge.reset(new Subset<uml::ActivityEdge, uml::Element >(m_ownedElement));//(m_ownedElement));
	
	
		/*SubsetUnion*/
		m_group.reset(new SubsetUnion<uml::ActivityGroup, uml::Element >(m_ownedElement));// ownedElement));
	
	
		/*SubsetUnion*/
		m_node.reset(new SubsetUnion<uml::ActivityNode, uml::Element >(m_ownedElement));// ownedElement));
	
	
		/*SubsetUnion*/
		m_ownedGroup.reset(new SubsetUnion<uml::ActivityGroup, uml::ActivityGroup >(m_group));// group));
	
	
		/*Subset*/
		m_ownedNode.reset(new Subset<uml::ActivityNode, uml::ActivityNode /*Subset does not reference a union*/ >(m_node));//(m_node));
	
	
		/*Subset*/
		m_partition.reset(new Subset<uml::ActivityPartition, uml::ActivityGroup
		,uml::ActivityGroup /*Subset does not reference a union*/ >(m_group,m_ownedGroup));//(m_group));
	
	
		/*Subset*/
		m_structuredNode.reset(new Subset<uml::StructuredActivityNode, uml::ActivityGroup
		,uml::ActivityNode /*Subset does not reference a union*/ >(m_group,m_node));//(m_group));
...
ActivityImpl::ActivityImpl(const ActivityImpl & obj)
{
	...
	(*(obj.getGroup().get())));// this->getGroup()->insert(this->getGroup()->end(), _group->begin(), _group->end());
	...
	(*(obj.getPart().get())));// this->getPart()->insert(this->getPart()->end(), _part->begin(), _part->end());
	 ...
	 _partition = obj.getPartition();
	m_partition.reset(new 		Subset<uml::ActivityPartition, uml::ActivityGroup
			,uml::ActivityGroup /*Subset does not reference a union*/ > 
	(*(obj.getPartition().get())));// this->getPartition()->insert(this->getPartition()->end(), _partition->begin(), _partition->end());
