...
ecore::EClass* ForkNodeActivationImpl::eStaticClass() const
{
...
// Operations
//*********************************
void ForkNodeActivationImpl::fire(std::vector<fUML::Token * > *  incomingTokens) 
{
    ...
    std::vector<Token *>* forkedTokens = new std::vector<Token *>();
    for (unsigned int i = 0; i < incomingTokens->size(); i++) {
        Token * token = incomingTokens->at(i);
        ForkedToken * forkedToken = fUML::FUMLFactory::eInstance()->createForkedToken();
