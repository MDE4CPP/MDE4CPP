...
void UnmarshallActionImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		...
		if ( nodeName.compare("result") == 0 )
		{
			...
			std::shared_ptr<uml::OutputPin> new_result = std::dynamic_pointer_cast<uml::OutputPin>(modelFactory->create(typeName, loadHandler->getCurrentObject(), uml::umlPackage::UNMARSHALLACTION_ATTRIBUTE_RESULT));
			if(new_result)
			{
...
void UnmarshallActionImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	...
	// Add references
	if ( this->eIsSet(package->getUnmarshallAction_Attribute_unmarshallType()) )
	{
