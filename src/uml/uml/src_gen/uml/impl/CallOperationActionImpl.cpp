...
// Operations
//*********************************
bool CallOperationActionImpl::type_target_pin(std::shared_ptr<Any> diagnostics, std::shared_ptr<std::map < Any, Any>> context)
{
	...
	switch(featureID)
	{
		case uml::umlPackage::CALLOPERATIONACTION_ATTRIBUTE_OPERATION:
		{
			...
			if(ecoreAny)
			{
				...
				{
					DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid type stored in 'ecore::ecoreAny' for feature 'operation'. Failed to set feature!"<< std::endl;)
					return false;
				}
			}
			else
			{
				DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid instance of 'ecore::ecoreAny' for feature 'operation'. Failed to set feature!"<< std::endl;)
				return false;
			}
		return true;
		}
...
std::shared_ptr<Any> CallOperationActionImpl::eInvoke(int operationID, std::shared_ptr<Bag<Any>> arguments)
{
	std::shared_ptr<Any> result;
  	...
  	switch(operationID)
	{
		// uml::CallOperationAction::type_target_pin(Any, std::map) : bool: 2210549299
