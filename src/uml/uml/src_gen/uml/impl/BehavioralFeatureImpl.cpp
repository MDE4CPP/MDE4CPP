...
std::shared_ptr<Any> BehavioralFeatureImpl::eInvoke(int operationID, std::shared_ptr<Bag<Any>> arguments)
{
  	...
  	switch(operationID)
	{
		...
		case umlPackage::BEHAVIORALFEATURE_OPERATION_ABSTRACT_NO_METHOD_EDIAGNOSTICCHAIN_EMAP:
		{
			...
			{
				DEBUG_ERROR("Invalid type stored in 'Any' for parameter 'diagnostics'. Failed to invoke operation 'abstract_no_method'!")
				return nullptr;
			}
			...
			{
				DEBUG_ERROR("Invalid type stored in 'Any' for parameter 'context'. Failed to invoke operation 'abstract_no_method'!")
				return nullptr;
			}
