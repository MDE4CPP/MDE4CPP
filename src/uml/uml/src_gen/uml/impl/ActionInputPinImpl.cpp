	...
	switch(featureID)
	{
		case uml::umlPackage::ACTIONINPUTPIN_ATTRIBUTE_FROMACTION:
		{
			...
			if(ecoreAny)
			{
				...
				{
					DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid type stored in 'ecore::ecoreAny' for feature 'fromAction'. Failed to set feature!"<< std::endl;)
					return false;
				}
			}
			else
			{
				DEBUG_MESSAGE(std::cout << __PRETTY_FUNCTION__ << " : Invalid instance of 'ecore::ecoreAny' for feature 'fromAction'. Failed to set feature!"<< std::endl;)
				return false;
			}
		return true;
		}
