...
void ParentedExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	...
	// Add references
	if ( this->eIsSet(package->getParentedExp_Attribute_innerExp()) )
	{
...
bool ParentedExpImpl::eSet(int featureID,  const std::shared_ptr<Any>& newValue)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::PARENTEDEXP_ATTRIBUTE_INNEREXP:
...
// EOperation Invoke
//*********************************
std::shared_ptr<Any> ParentedExpImpl::eInvoke(int operationID, const std::shared_ptr<Bag<Any>>& arguments)
{
